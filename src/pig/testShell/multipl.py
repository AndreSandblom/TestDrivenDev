...
class MultiGame(cmd.Cmd):
    ...
    def __init__(self):
        ...
        self.dice = Dice(6)
        self.hsb = Highscore("multiGame.bin")
    ...
    def do_set_name(self):
        ...
        if self.whose_turn == 1:
            ...
            self.player2.change_name(name)

    def do_roll(self):
        """Type in "roll" to roll a dice."""
        rolled = self.dice.roll()
        if self.whose_turn == 1:
            self.increment_and_determine_1(rolled)
        else:
            self.increment_and_determine_2(rolled)
